...
image_path = "C:/Users/User/Desktop/side_project_data/Pictures_preprocessed/First/"
...
RMSE = []
PT_num = []
df = pd.read_csv("C:/vscode/age_estimation_forlab/best_model_morph_50models.csv")
csv_filename = df["CSV"].values
Models = df["Model"].values
#df1 = pd.read_csv("C:/vscode/age_estimation_forlab/PT_sorted_age.csv")
df1 = pd.read_csv("C:/vscode/age_estimation_forlab/patient-revised.csv")
fname = df1["filename1"].values
ages = df1['age1'].values
pt_num = df1['Patient_num'].values
...
for i,name in enumerate(csv_filename) :
    #best model names
    md = os.path.join(ROOT,f'TF_model/best_model{Models[i]}.pt')
    #different CSVs
    ...
    # ages = df1['age'].values
    for j,names in enumerate(fname):
        if type(names) == str:
        #groundTruth_ages.append(ages[i])
            ...
            if ages[j] < 76: #above70 use 70-52='18' 71-52=19
                ct+=1
...
d = {'Patient_num':PT_num,'filename':ALL,"model":mod,"error":err,"predicted":pred,"groundTruth":groundTruth_ages,"MAE":MAE,"RMSE":RMSE}
fl = pd.DataFrame(data=d)
fl.to_csv('C:/vscode/age_estimation_forlab/Error_patient_under76_50models_527.csv')
