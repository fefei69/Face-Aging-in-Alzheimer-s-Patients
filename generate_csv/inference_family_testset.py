...
df = pd.read_csv("C:/vscode/age_estimation_forlab/best_model_morph_40to50_models.csv")
...
for i,name in enumerate(csv_filename) :
    ...
    for j in range(len(ages)):
        ...
        if ages[j] > 18: #70-52=18 71-52=19
            ct+=1
...
print(len(ALL),len(mod),len(err),len(pred),len(groundTruth_ages),len(MAE),len(RMSE))
fl = pd.DataFrame(data=d)
fl.to_csv('C:/vscode/generate_csv/Error_analysis_csv_results/Error_family_testset_above70_40to50models.csv')
...
# plt.legend()
# plt.savefig("family_testset_under70_histo.png")
# plt.show()

# plt.scatter(groundTruth_ages, pred , s = 5, c="b" , marker = 'o', label ='family')
...
# plt.legend()
# plt.savefig("family_testset_under70.png")
# plt.show()
